...
本报告按 `DEVELOPMENT.md` 在 `debian-13-dev` 中启动 vfio-user server，并在嵌套 VM 中重新构建、加载驱动和本仓库 rdma-core provider 后运行 `test_data_transfer`。本轮记录对应提交 `6c038aa`，后端为 `loopback:mode=preserve`。
...
本轮 `rmmod rocm_ernic_rdma rocm_ernic_eth` 为 328 ms，证明 AdminQ IRQ 在资源注销期间仍可用，已修复旧代码中的约 60 秒五/六次超时问题。
